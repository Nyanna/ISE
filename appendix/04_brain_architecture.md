...
A further consequence: an engram cannot be teleported. If you relocate a synaptic modification from one network position to another, you change the activation path through which it is traversed, and therefore you change its meaning. What arrives at the new location is necessarily a *different* engram — not the same engram at a new address.

This is not the impossibility of systems consolidation but its correct characterization: what is called consolidation is the formation of a new engram at a new network position through undirected re-reading. The original engram is not destroyed in this process. It persists physically — optogenetic reactivation of silent hippocampal engrams in remote-memory experiments confirms that the original configuration remains structurally intact and can still drive behavior when bypassing the natural traversal mechanism (Park et al., 2026; Lei et al., 2025; Ko et al., 2025). What changes is gain dominance: a new neocortical engram, formed by repeated traversal, acquires higher gain under the evolved interpreter and thereby overlays the original in natural retrieval. The hippocampal engram is not erased — it is silenced by competitive gain dynamics. The observed transition from episodic to semantic memory is the predicted signature of positional drift: a neocortical engram at a coarser topology encodes gist rather than episode, because the cortical architecture at that position cannot support the activation-path granularity of the hippocampal original. This has direct implications for systems consolidation theory, addressed below.
...
Sharp-wave ripples in the hippocampus during sleep — the primary evidence cited for active consolidation — are not directed traversals in the interpreter sense, but they are not merely passive residual activations either. They are undirected, gain-unsteered replays mediated by the same thalamocortical infrastructure that operates without directed gain modulation during NREM. The thalamic coordinator is not performing conscious traversal — the full thalamocortical feedback loop does not run with directed gain steering — but the thalamic hardware participates in coordinated hippocampo-cortical communication (via nucleus reuniens and thalamocortical spindle-SWR coupling) that enables undirected re-reading of recently formed configurations. The correlation between ripple activity and subsequent memory performance reflects encoding quality: strong encoding produces both more coordinated replay (more ripples with thalamic coupling) and more durable engrams (broader structural footprint). But the replay is not an active selection process — it is gain-biased by the residual amplitude of recently formed configurations, with high-gain encodings generating stronger and more coherent ripple sequences. The new neocortical engram formed through this undirected re-reading is a byproduct: a coarser-resolution representation deposited in situ as the activation reaches cortical regions, whose topology cannot encode hippocampal episodic granularity. The hippocampal original is not transferred and not erased — it is gain-overlaid.
...
Consolidation in the sense of active engram rewriting or selective stabilization does not occur. What does occur — continuously, during waking and sleep alike — is undirected, gain-biased re-reading: the thalamocortical loop traverses recently formed configurations without directed gain steering whenever the coordinator is not occupied with external processing. During waking, this undirected re-reading is masked by the ongoing stream of new experience and directed processing. During NREM sleep, directed gain modulation ceases and noise is reduced, allowing the undirected re-reading to proceed more efficiently and selectively, biased toward high-gain encodings from the preceding waking period. What occurs during practice is chain extension and depth compression through conscious traversal. What occurs during sleep is both hardware maintenance and the low-noise continuation of gain-biased undirected re-reading — the same process, distinguished by noise floor rather than mechanism.
...
**NREM sleep** is the phase in which directed gain modulation is absent and the most metabolically expensive maintenance occurs — synaptic homeostasis, glymphatic clearance of metabolic waste products, protein turnover in engram-bearing synaptic structures. The coordinator does not perform directed conscious traversal; it operates in an undirected, low-noise mode. Thalamocortical spindles (9–15 Hz, generated in the thalamic reticular nucleus) continue to propagate thalamo-cortical coupling, but without the selective gain steering that characterizes waking traversal. This infrastructure supports the undirected re-reading described above. Slow oscillations facilitate glymphatic flow and synchronize metabolic processes across large tissue volumes. The depth of NREM — its resistance to arousal — reflects the priority of structural maintenance: the system actively suppresses directed coordinator reactivation to protect the maintenance window.
...
Chemical storage → semantics in network → interpreter → path-based meaning → universal components with positional semantics → identity of algorithm and memory → cortical architecture determines function → consolidation as undirected gain-biased re-reading with positional drift → ontogenesis as chain building → identity as interpreter state → culture as synchronized traversal → empathy as gain coupling → creativity as gain window expansion → aging as interpreter drift → thalamic self-reference → missing difference signal → phenomenal consciousness.
