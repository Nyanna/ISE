...
**Unified Expansion as a Working Premise**

Taking a single expansion mechanism as the working premise shifts the question from *how to bridge inflation and dark energy* to *how a single driver manifests across different scales*. If the same force has been acting on the universe’s expansion throughout its entire history, the split into an inflationary epoch and a dark-energy-dominated phase becomes a description of regimes rather than of distinct phenomena. Read this way, cosmic expansion evolves continuously with the universe’s changing spatial scales, and the transition between regimes is a change in effective intensity rather than a change of mechanism. This is an interpretive choice, not a derivation: both the separated-phase and unified readings remain compatible with current data, and the argument here is one of parsimony — a single driver whose effect scales with the universe’s state avoids positing a dedicated inflationary field that has no further role after reheating.
